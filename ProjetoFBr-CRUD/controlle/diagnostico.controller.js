const Model = require('../models/diagnostico.model');

async function criar(req, res) {
  try {
    const result = await Model.criar(req.body);
    res.status(201).json({ mensagem: 'Diagnóstico registrado com sucesso!', id: result.insertId });
...
async function buscarPorId(req, res) {
  try {
    ...
    if (!dado) return res.status(404).json({ mensagem: 'Diagnóstico não encontrado' });
    res.json(dado);
  } catch (erro) { res.status(500).json({ erro: erro.message }); }
}
...
async function atualizar(req, res) {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ mensagem: 'Diagnóstico não encontrado' });
    res.json({ mensagem: 'Diagnóstico atualizado com sucesso!' });
  } catch (erro) { res.status(500).json({ erro: erro.message }); }
}
...
async function deletar(req, res) {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ mensagem: 'Diagnóstico não encontrado' });
    res.json({ mensagem: 'Diagnóstico deletado com sucesso!' });
  } catch (erro) { res.status(500).json({ erro: erro.message }); }
}
