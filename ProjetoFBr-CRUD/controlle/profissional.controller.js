...
async function criar(req, res) {
  try {
    ...
    res.status(201).json({ mensagem: 'Profissional criado com sucesso!', id: result.insertId });
...
async function buscarPorId(req, res) {
  try {
    ...
    if (!dado) return res.status(404).json({ mensagem: 'Profissional não encontrado' });
    res.json(dado);
  } catch (erro) { res.status(500).json({ erro: erro.message }); }
}
...
async function atualizar(req, res) {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ mensagem: 'Profissional não encontrado' });
