const Model = require('../models/exame.model');

async function criar(req, res) {
  try {
    const result = await Model.criar(req.body);
    res.status(201).json({ mensagem: 'Exame solicitado com sucesso!', id: result.insertId });
...
async function buscarPorId(req, res) {
  try {
    ...
    if (!dado) return res.status(404).json({ mensagem: 'Exame não encontrado' });
    res.json(dado);
  } catch (erro) { res.status(500).json({ erro: erro.message }); }
}
...
async function atualizar(req, res) {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ mensagem: 'Exame não encontrado' });
